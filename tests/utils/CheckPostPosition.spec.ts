...
describe("utils/CheckPostPosition", () => {
  ...
  describe("알맞지 않은 조사가 들어갔을 때", () => {
    it("상희이 일 때", () => {
      expect(checkPostPosition("상희이", ["이", "가"])).to.be.null;
    });

    it("변수이랑 일 때", () => {
      expect(checkPostPosition("변수이랑", ["이랑", "랑"])).to.be.null;
    });

    it("변수 일 때", () => {
      expect(checkPostPosition("변수", ["이랑", "랑"])).to.be.null;
    });
  });
  ...
  describe("두번째 조사가 더 길 때", () => {
    ...
    it("알맞은 조사일 때", () => {
      expect(checkPostPosition("안녕나가", ["가", "나가"])).to.be.null;
    });
